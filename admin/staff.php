...
<?php
    ...
    if (isset($_POST['update_button'])) {
        ...
        $sql1 = "UPDATE Staff SET firstName=?, lastName=?, email=?, phone=? WHERE userId=?";
        //Prepared Statement
        $stmt =  $db->stmt_init();
        $stmt->prepare($sql1);
        $stmt->bind_param('ssssd', $firstName, $lastName, $email, $phone, $userId);
        $stmt->execute();
        ...
        $sql1 = "DELETE FROM Staff WHERE userId=?";
        //Prepared Statement
        $stmt =  $db->stmt_init();
        $stmt->prepare($sql1);
        ...
        echo '<script type="text/javascript">
           window.location.replace("staff-update-delete.php");
                </script>';
...
<body>
    <div class="container profile profile-view" id="profile">
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . "?staffID=" . $_GET['staffID'];?>">
            <div class="row profile-row">
                <div class="col-md-12">
                    ...
                    <div class="form-group">
                        <label class="control-label">Phone Number (format: x-xxx-xxx-xxxx) </label>
                        <input class="form-control" type="text" pattern="^\d{1}-\d{3}-\d{3}-\d{4}$" required="" autocomplete="off" name="phone" value='<?php echo $row['phone']; ?>'>
                    </div>
                    <div class="row">
